...
#define PROC_NAME "cache_timing_attack"
...
static void perform_cache_timing_attack(void)
{
    ...
    pr_info("[CACHE_TIMING] ========================================\n");
    pr_info("[CACHE_TIMING] Cache Timing Side-Channel Attack Started\n");
    pr_info("[CACHE_TIMING] Cache Line Size: %d bytes\n", CACHE_LINE_SIZE);
    pr_info("[CACHE_TIMING] Probes: %d\n", NUM_PROBES);
    pr_info("[CACHE_TIMING] Threshold: %d cycles\n", THRESHOLD_CYCLES);
    pr_info("[CACHE_TIMING] Starting cache analysis...\n");
    ...
    pr_info("[CACHE_TIMING] All cache lines flushed\n");
    
    // Probe cache state
    pr_info("[CACHE_TIMING] Probing cache state (%d probes)...\n", NUM_PROBES);
    for (i = 0; i < NUM_PROBES; i++) {
    ...
    pr_info("[CACHE_TIMING] Analysis complete\n");
    pr_info("[CACHE_TIMING] Cache Hits: %u\n", probe->hit_count);
    pr_info("[CACHE_TIMING] Cache Misses: %u\n", probe->miss_count);
    if ((probe->hit_count + probe->miss_count) > 0) {
        u32 hit_rate = (probe->hit_count * 100) / (probe->hit_count + probe->miss_count);
        pr_info("[CACHE_TIMING] Hit Rate: %u%%\n", hit_rate);
    }
    pr_info("[CACHE_TIMING] ========================================\n");
}
                         ...
                         size_t count, loff_t *ppos)
{
    ...
    len = snprintf(buffer, sizeof(buffer),
        ...
        probe->hit_count,
        probe->miss_count,
        probe->hit_count * 100 / (probe->hit_count + probe->miss_count));

    // Show indices with cache hits
    int i;
    for (i = 0; i < NUM_PROBES && len < (int)(sizeof(buffer) - 50); i++) {
