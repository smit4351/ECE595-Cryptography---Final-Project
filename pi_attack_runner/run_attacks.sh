...
print_header() {
    ...
    echo -e "${BLUE}$1${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" >> "${RESULTS_DIR}/run.log" 2>/dev/null || true
}

print_ok() {
    echo -e "${GREEN}[OK]${NC} $1"
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [OK] $1" >> "${RESULTS_DIR}/run.log" 2>/dev/null || true
}

print_error() {
    echo -e "${RED}[ERROR]${NC} $1" >&2
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [ERROR] $1" >> "${RESULTS_DIR}/run.log" 2>/dev/null || true
}

print_warning() {
    echo -e "${YELLOW}[WARN]${NC} $1"
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [WARN] $1" >> "${RESULTS_DIR}/run.log" 2>/dev/null || true
}

print_info() {
    echo -e "${BLUE}[INFO]${NC} $1"
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [INFO] $1" >> "${RESULTS_DIR}/run.log" 2>/dev/null || true
}
...
main() {
    ...
    print_info "Collecting kernel logs for partner communication..."
    
    # Save dmesg output
    dmesg > "${RESULTS_DIR}/dmesg.log" 2>&1 || true
    print_ok "Kernel logs saved to: ${RESULTS_DIR}/dmesg.log"
    
    # Run log parser if available
    if command -v "${SCRIPT_DIR}/log_parser.sh" &> /dev/null; then
        "${SCRIPT_DIR}/log_parser.sh" "${RESULTS_DIR}" || true
    fi
    
    # Run log collector if available
    if command -v "${SCRIPT_DIR}/collect_logs.sh" &> /dev/null; then
        "${SCRIPT_DIR}/collect_logs.sh" "${RESULTS_DIR}/collected_logs.txt" || true
    fi
    ...
    echo ""
    echo "To send results to your partner:"
    echo "  cat ${RESULTS_DIR}/attack_report_*.txt"
    echo "  cat ${RESULTS_DIR}/dmesg.log"
    echo "  cat ${RESULTS_DIR}/collected_logs.txt"
}
