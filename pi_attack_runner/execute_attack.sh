...
execute_via_proc() {
    ...
    local target_cmd="target:$target_addr"
    if [[ "$MODULE_NAME" == "dma_attack" ]]; then
        target_cmd="target=$target_addr"
    fi
    
    if echo "$target_cmd" > "$proc_interface" 2>/dev/null; then
    ...
    if [[ "$MODULE_NAME" == "peripheral_isolation_test" ]]; then
        start_cmd="test all"
    elif [[ "$MODULE_NAME" == "dma_attack" ]]; then
        start_cmd="execute"
    fi

    if ! echo "$start_cmd" > "$proc_interface" 2>&1; then
    ...
    local lines_before=$(wc -l < "$dmesg_before_file")
    local new_lines=$(tail -n +$((lines_before + 1)) "$dmesg_after_file")
...
REQUIRED:
  --module NAME           Attack module name (e.g., dma_attack, cache_timing_attack)
...
EXAMPLES:
  ...
  $0 --module cache_timing_attack --timeout 120 --verbose
